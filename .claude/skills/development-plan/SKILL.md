...
2. **Scaffolding de Microservicios y Frontend**
 ...
 4. **Configuración Inicial Post-Scaffold** — indicar que el script ejecuta automáticamente el paso 9 (`create-all-secrets-dev.sh`); **no hay ajuste manual posterior**: el script es completamente autónomo:
 ...
 6. **Criterios de Aceptación** — lista de verificación; el criterio principal es `bash .claude/scripts/scaffold-all-services.sh finalizó los 12 pasos con código de salida 0`. Incluir también: `Los repositorios ECR de todos los microservicios existen en floci (verificado en paso 11)`, `Los secrets flexicredit/dev/<servicio> existen en floci con los valores reales de Terraform (verificado en paso 12)`, `.env.local del frontend creado con outputs de Terraform`.
