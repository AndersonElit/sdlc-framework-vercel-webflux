...
4. **PostgreSQL — Migraciones Flyway por Microservicio**
   ...
   - Regla Database-per-Service: las migraciones de cada servicio aplican sobre **su propia BD** (`<prefijo>_<servicio_slug>`), no sobre una BD compartida; Flyway las aplica automáticamente al arrancar el servicio
...
2. **Scaffolding de Microservicios y Frontend**
 ...
 3. **Generación de Migraciones Flyway** — indicar explícitamente que estas secciones son **ejecutadas de forma automática por `scaffold-all-services.sh`** como pasos 5 y 6 del script, inmediatamente después del scaffold (cuando los directorios de los servicios ya existen); la sección es informativa de lo que el script hace, no pasos manuales:
    - **Paso 5 — V1 Flyway por microservicio**: para cada servicio incluido en `--bc-tags`, extrae el bloque `-- BC-XX:` correspondiente del `schema.sql` y lo escribe en `backend/<servicio>/rest-api/src/main/resources/db/migration/V1__initial_schema.sql`; si el archivo ya existe lo omite (idempotente); si no se pasó `--bc-tags`, este paso se salta.
    - **Paso 6 — V2 seed de seguridad-service**: si `backend/seguridad-service` existe, genera `V2__seed_roles_permisos.sql` con los 7 roles del sistema, permisos por bounded context y el mapeo `roles_permisos`; si el archivo ya existe lo omite.
